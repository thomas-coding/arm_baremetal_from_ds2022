...
/* Shared data */
volatile unsigned int total_time_CPU0 = 0, total_time_CPU1 = 0;
volatile unsigned int sync_1 = 0;
...
__attribute__((noreturn)) void main_app(void)
{
    ...
    /* Run shell_sort and qsort in CORE1 */
    if (CORE1 == id)
    {
        /* Do shell sort */
        memcpy(strings_copy, strings, sizeof(strings));
        ...
        printf("Quick sort took %d clock ticks in CORE1\n", endtime - starttime);
        total_time_CPU1 += endtime - starttime;
        sync_1 = 1;
    }
