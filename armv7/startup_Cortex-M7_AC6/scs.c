...
void SCS_init(void)
{
    ...
    SCS.MPU.RegionAttrSize = RO | TEXSCB | REGION_16K | REGION_ENABLED;
    ...
    SCS.MPU.RegionAttrSize = FULL_ACCESS | TEXSCB | REGION_8K | REGION_ENABLED;

    /* Configure a region to cover Heap and Main Stack (Not Executable, Read-Write) */
#ifdef TWO_REGION
    SCS.MPU.RegionBaseAddr = ((unsigned int) &Image$$ARM_LIB_STACK$$Base) | REGION_VALID | 3;
#else
    SCS.MPU.RegionBaseAddr = ((unsigned int) &Image$$ARM_LIB_STACKHEAP$$Base) | REGION_VALID | 3;
#endif
    SCS.MPU.RegionAttrSize = NOT_EXEC | FULL_ACCESS | TEXSCB | REGION_4K | REGION_ENABLED;
