...
{
/* IMPORTANT */
/* To capture ITM data with the Debugger, select Run->Debug Configurations, then select the smp_primes_ITM_AC6-versatile-A9x4 debug config.
In its Connections panel, press 'DTSL Options Edit...'.  In the 'Trace Capture' tab, select either 'On Chip Trace Buffer (ETB)' or
(for DSTREAM-ST) 'Streaming Trace' or (for DSTREAM) 'DSTREAM 4GB Trace Buffer'.  In the 'ITM' tab, tick 'Enable ITM Trace'.
Ensure 'The target will setup the ITM' is selected, with ATBID=1.  Click on OK to save the DTSL options.  For more information, see the readme.html */
