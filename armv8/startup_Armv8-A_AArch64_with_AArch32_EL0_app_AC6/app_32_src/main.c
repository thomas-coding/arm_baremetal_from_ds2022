...
// Once scatter loading is done in EL0, we need to perform a syscall #1 to EL1
// to enable cache in EL1
__attribute__((used)) void _platform_pre_stackheap_init(void)
{
	asm volatile ("svc #1");  //system call
}
...
{
    return user_perthread_libspace[EL0GetCPUID()];
}
...
__attribute__((noreturn)) int main(void)
{
    ...
    printf("\r\nArmv8-A SMP Prime Number Generator Example, using Arm Compiler for Embedded 6\r\n\r\n");
