 ...
 */
static GICv3_GICR gicrbase[2] __attribute__((section (".bss.gicr")));
...
static inline GICv3_redistributor_RD * getgicrRD(uint32_t gicr)
{
    return &(gicrbase[gicr].RDblock.RD_base);
}

static inline GICv3_redistributor_SGI * getgicrSGI(uint32_t gicr)
{
    return &(gicrbase[gicr].SGIblock.SGI_base);
}
